...
/**
 * Voice26 Professional Audio Engine
 *
...
export class Voice26AudioEngine {
  ...
  async initialize(): Promise<void> {
    ...
    console.log('[Voice26Engine] Initialized with 48kHz sample rate');
  }
  ...
  async connectToAudio(): Promise<boolean> {
    ...
    if (!audio) {
      console.log('[Voice26Engine] No audio element, watching...');
      this.watchForAudio();
      return false;
    }
  ...
  private findAudioElement(): HTMLAudioElement | null {
    ...
    for (const audio of audios) {
      if ((audio as any).__voice26Connected) continue;
      if (audio.src || audio.srcObject) return audio;
    }
    return null;
  }
  ...
  private connectElement(audio: HTMLAudioElement): boolean {
    ...
    try {
      ...
      (audio as any).__voice26Connected = true;
      ...
      console.log('[Voice26Engine] Connected to audio');
      return true;
    } catch (err) {
      console.error('[Voice26Engine] Connection failed:', err);
      return false;
    }
  }
  ...
  applyPreset(presetId: string): void {
    ...
    if (preset) {
      ...
      console.log(`[Voice26Engine] Applied preset: ${preset.name}`);
    }
  }
  ...
  disconnect(): void {
    ...
    if (this.audioElement) {
      (this.audioElement as any).__voice26Connected = false;
...
let engineInstance: Voice26AudioEngine | null = null;

export function getVoice26Engine(): Voice26AudioEngine {
  if (!engineInstance) {
    engineInstance = new Voice26AudioEngine();
  }
  return engineInstance;
}

export default Voice26AudioEngine;
